...
func init() {
	android.RegisterModuleType("wings_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = wingsExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = wingsExpandVariables(ctx, dep_file)
	...
	cmd := wingsExpandVariables(ctx, String(g.properties.Cmd))
